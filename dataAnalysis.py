...
def fill_and_flag(series, max_gap=10, n_neighbors=4):
    ...
    for start, end in gaps:
        if (end - start + 1) <= max_gap:
            ...
            seg = s.iloc[left:right+1]
            s.iloc[left:right+1] = seg.interpolate()
...
if st.sidebar.button("Load Data"):
    ...
    times = device_dfs[master].sort_values('Timestamp')['Timestamp']

    records = []
    for dev, df in device_dfs.items():
        tmp = df.set_index('Timestamp').reindex(
            times, method='nearest', tolerance=pd.Timedelta(minutes=30)
        )
...
if st.sidebar.button("Analyze"):
    if 'df_all' not in st.session_state:
        ...
        st.altair_chart((base_t.mark_line() + base_t.transform_filter('Interpolated').mark_circle(color='red')), use_container_width=True)
        ...
        st.altair_chart((base_r.mark_line() + base_r.transform_filter('Interpolated').mark_circle(color='red')), use_container_width=True)
        ...
        st.altair_chart(alt.Chart(df_norm).mark_line().encode(
            x=alt.X('Timestamp:T', axis=alt.Axis(format='%m/%d')),
        ...
        df_ct = corr_t.reset_index().melt('Device', var_name='Device2', value_name='Corr')
        st.altair_chart(alt.Chart(df_ct).mark_rect().encode(
        ...
        df_cr = corr_rh.reset_index().melt('Device', var_name='Device2', value_name='Corr')
        st.altair_chart(alt.Chart(df_cr).mark_rect().encode(
            x='Device2:O', y='Device:O', color='Corr:Q'
        ), use_container_width=False)
    ...
    st.info("Use 'Load Data' and 'Analyze' to run the analysis.")
